...
if (!function_exists('pagination_helper')) {
    /**
     * Load the PaginationHelper class
     */
    function pagination_helper()
    {
        return new \App\Libraries\PaginationHelper();
    }
}

if (!class_exists('PaginationHelper')) {
    /**
    ...
    class PaginationHelper
    {
         ...
         */
        public static function generateLinks(array $pagination, string $baseUrl, array $queryParams = []): array
        {
            $links = [];
            
            // First page link
            if ($pagination['show_first_last'] && $pagination['has_previous']) {
                $links['first'] = [
            ...
            // Page number links
            if ($pagination['show_page_numbers']) {
            ...
            // Last page link
            if ($pagination['show_first_last'] && $pagination['has_next']) {
                $links['last'] = [
                    'url' => self::buildUrl($baseUrl, $pagination['last_page'], $queryParams),
         ...
         */
        public static function getPageRange(int $currentPage, int $totalPages, int $range = 3): array
        {
         ...
         */
        public static function buildUrl(string $baseUrl, int $page, array $queryParams = []): string
        {
         ...
         */
        public static function generateMetadata(int $currentPage, int $perPage, int $totalRecords, array $config = []): array
        {
            // Ensure minimum values
            $currentPage = max(1, $currentPage);
         ...
         */
        public static function validatePage($page, int $totalPages = 1): int
        {
            // Convert to integer
            $page = (int)$page;
